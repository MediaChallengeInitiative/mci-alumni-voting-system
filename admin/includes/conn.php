...
if ($conn->connect_error) {
    ...
    // Log detailed error
    $log_message = date('[Y-m-d H:i:s]') . " Database Connection Error (Admin)\n";
...
$tables_check = $conn->query("SHOW TABLES LIKE 'admin'");
if ($tables_check && $tables_check->num_rows == 0) {
    error_log("Warning: 'admin' table not found in database. Schema may not be imported.");
}
?>
